...
class MealMasterParser(BaseRecipeParser):
    ...
    def parse_content(self, content: str, filepath: str) -> List[Recipe]:
        ...
        for line in lines:
            ...
            if (line_str.startswith('MMMMM') or line_str.startswith('-----')) and 'Recipe via' in line_str:
                self._save_pending(recipes, current_recipe, current_ingredient_raw, current_instruction_paragraph)
    ...
    def _save_pending(self, recipes: List[Recipe], current_recipe: Recipe, current_ingredient_raw: List[str], current_instruction_paragraph: List[str]):
